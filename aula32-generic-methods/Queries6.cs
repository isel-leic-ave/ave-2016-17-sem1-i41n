...
static class App
{
    ...
    static IEnumerable<R> Convert<T, R>(this IEnumerable<T> src, Func<T,R> transf) {
    ...
    static IEnumerable<T> Filter<T>(this IEnumerable<T> src, Predicate<T> p) {
        foreach(T item in src) {
            if(p(item))
                yield return item;
        }
    ...
    static void Main()
    {
        ...
        */
        // ??? Se os métodos Convert, Filter e Distinct fossem métodos de instãncia de IEnumerable
        
        IEnumerable names = Lines("i41n.txt")
            .Convert(Student.Parse)
            .Filter(s => {  return s.nr > 38000; })
            // .Filter(s => {  return s.name.StartsWith("A"); })
            .Convert( s => { return s.name.Split(' ')[0]; } )
            .Distinct();
